...
description: "Understand the Resume Builder monorepo, runtime boundaries, and package ownership model"
---

Resume Builder is a pnpm/Turborepo monorepo. The product runs as one deployed Node.js process, while the source is split into a full-stack web app, a server adapter, and focused internal packages.
...
flowchart TD
  ...
  subgraph NodeProcess["Node process"]
    ...
    Auth["packages/auth"]
    MCP["packages/mcp"]
    PDFServer["@resume-builder/pdf/server"]
  end
...
- Use `@resume-builder/*` package exports for cross-workspace imports.
- Do not import another workspace through `apps/**`, `packages/**`, `@resume-builder/*/src/**`, or a TypeScript path alias to another workspace's `src`.
...
- `@resume-builder/pdf/browser` creates browser PDF blobs.
- `@resume-builder/pdf/server` creates server PDF files.
...
The server process imports MCP from `@resume-builder/mcp` and injects the in-process oRPC router client. It must not import MCP code from `apps/web/src`.
