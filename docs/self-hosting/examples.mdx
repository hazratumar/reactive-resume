...
<Info>
  **Help others by sharing your setup!** If you have a working configuration that isn't covered here, I'd love to
  include it. Simply [open a pull request](https://github.com/amruthpillai/resume-builder) with your example added to
...
This example uses [Traefik](https://traefik.io/) as a reverse proxy with automatic SSL certificate management via Let's Encrypt. Only the Resume Builder app is exposed through Traefik—Postgres remains on an internal network.
...
services:
  ...
  reactive_resume:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.resume-builder.rule=Host(`resume.${DOMAIN}`)"
      - "traefik.http.routers.resume-builder.entrypoints=websecure"
      - "traefik.http.routers.resume-builder.tls.certresolver=letsencrypt"
      - "traefik.http.services.resume-builder.loadbalancer.server.port=3000"
    healthcheck:
...
services:
  ...
  reactive_resume:
    ...
    deploy:
      ...
      labels:
        - "traefik.enable=true"
        - "traefik.http.routers.app.rule=Host(`resumebuilder.com`)"
...
To contribute, [open a pull request](https://github.com/amruthpillai/resume-builder) with your example added to this page. Include:
