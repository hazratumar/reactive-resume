...
Resume Builder supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  ...
  [resumebuilder.com](https://resumebuilder.com), you can use the built-in Google and GitHub sign-in options.
</Info>
...
<Tabs>
  <Tab title="Option A: OIDC Discovery (Recommended)">
    ...
    **Examples:**
    - Authentik: `https://auth.example.com/application/o/resume-builder/.well-known/openid-configuration`
...
Resume Builder automatically maps user profile data from the OAuth provider. The following fields are used:

| Resume Builder Field | OAuth Profile Fields (in order of preference) |
| --------------------- | --------------------------------------------- |
...
OAUTH_DISCOVERY_URL="https://auth.example.com/application/o/resume-builder/.well-known/openid-configuration"
```
  </Step>
</Steps>
...
OAUTH_CLIENT_ID="resume-builder"
OAUTH_CLIENT_SECRET="your-plain-secret"
OAUTH_DISCOVERY_URL="https://auth.example.com/.well-known/openid-configuration"
```

    <Warning>
      Use the **plain text** secret in Resume Builder's environment, not the hashed version used in Authelia's configuration.
    </Warning>
...
<Steps>
  <Step title="Create a client">
    ...
    3. Set **Client ID** (e.g., `resume-builder`)
...
OAUTH_CLIENT_ID="resume-builder-client-id"
OAUTH_CLIENT_SECRET="resume-builder-client-secret"
OAUTH_DISCOVERY_URL="https://auth.company.com/application/o/resume-builder/.well-known/openid-configuration"
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your Resume Builder instance and OAuth provider in production. OAuth tokens should never
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for Resume Builder.
  </Card>
</CardGroup>
