...
description: "Learn which file formats Resume Builder can import and how to import an existing resume from the dashboard."
---

Resume Builder can create a new resume from several existing file formats. Use import when you are moving from another tool, restoring a backup, or converting an older Resume Builder file.

## Supported import formats

| Format | Requires AI integration? | Notes |
| --- | --- | --- |
| **Resume Builder (JSON)** | No | Best option for backups exported from the current version of Resume Builder. |
| **Resume Builder v4 (JSON)** | No | Use this for files exported from Resume Builder v4. |
| **JSON Resume** | No | Use this for files that follow the JSON Resume schema. |
| **PDF** | Yes | Resume Builder asks your configured AI provider to parse the file into structured resume data. |
| **Microsoft Word** | Yes | Supports Word documents. Resume Builder asks your configured AI provider to parse the document. |
...
<Steps>
  ...
  <Step title="Import">
    Click **Import**. Resume Builder creates a new resume and opens it in the builder when the import succeeds.
  </Step>
</Steps>

## Choose the right import type

If you have a file exported from Resume Builder, choose **Resume Builder (JSON)**. This preserves the most information because the file already matches Resume Builder's data model.

If you are coming from Resume Builder v4, choose **Resume Builder v4 (JSON)**.
...
Make sure you selected the correct JSON import type. A Resume Builder JSON export, a Resume Builder v4 JSON export, and a JSON Resume file are different formats.
