...
description: "Learn about the Resume Builder JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
Resume Builder aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from Resume Builder as a JSON file
...
The Resume Builder JSON Schema is publicly available at:

<Info>[https://resumebuilder.com/schema.json](https://resumebuilder.com/schema.json)</Info>

This endpoint returns a complete JSON Schema (draft 07 compatible) that describes the structure of a Resume Builder document. It includes:
...
```json
{
  "$schema": "https://resumebuilder.com/schema.json",
  "version": "5.0.0"
}
```

When importing resumes, Resume Builder uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("https://resumebuilder.com/schema.json").then((r) => r.json());
...
- **Job portals** can accept Resume Builder JSON files directly
...
Below is the complete JSON Schema for Resume Builder. You can also fetch the latest version directly from [https://resumebuilder.com/schema.json](https://resumebuilder.com/schema.json).
