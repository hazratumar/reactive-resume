...
describe("SEO static endpoints", () => {
	it("generates robots.txt from the normalized app URL", async () => {
		...
		expect(text).toContain("Sitemap: https://docs.resumebuilder.com/sitemap.xml");
	...
	it("generates an app-domain-only sitemap", async () => {
		...
		expect(text).not.toContain("docs.resumebuilder.com");
	...
	it("generates a lightweight llms.txt product index", async () => {
		...
		expect(text).toContain("- Documentation llms.txt: https://docs.resumebuilder.com/llms.txt");
		expect(text).toContain("- API documentation: https://docs.resumebuilder.com/api-reference");
		expect(text).toContain("- Resume schema: https://app.example.com/schema.json");
		expect(text).toContain("- MCP documentation: https://docs.resumebuilder.com/guides/using-the-mcp-server");
